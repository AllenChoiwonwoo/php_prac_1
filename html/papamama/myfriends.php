...
*/
  ...
  // if ($page > $pageNum) {// get 값이 더 클경우
  //   $pageNum = $pageNum-1;
  //   // code...
  // }

  if ($page >= $pageNum ) {//get으로 넘어 page값이 총페이지 개수 보다 크면, page에 총페이지 값 넣어주기 위한 조건문
    // code...
    $page = $pageNum-1;
  }
  // if ($nowBlock >= $blockNum && $page >= $pageNum) {
  //   // code...
  //
  // }
  ...
  if ($s_page >= $pageNum) {// 만약 $s_page 값이 총 페이지수보다 크다면 마지막 블록의 첫페이지로 값을 바꿔주기 위한 조건문
    // code...
    $s_page = ($blockNum)*$block-($block-1);
  }//시작페이지는
  ...
  // echo "<br> 현제 페이지의 첫 친구번호 ".$friends_numb;
  // echo "<br> 총 게시물".$num;
  // echo "<br> list = ".$list;
        ...
        <table class="table table-striped table-sm">
          ...
          <tbody>
            <tr>
              <?php
                echo "<br>".$page."<br>";
                $friends_numb = ($page*$list)-4; //친구의 순서를 표시해준다.
                $s_point = ($page-1)*$list;//DB에서 값을 가져오기를 시작하는 지점을 정하는 코드다.
                //page값을 이용하여 시작지점을 구해야 그로부터 list개의 데이터를 불러와 화면에 표시할 수 있다.
                $query = 'SELECT mail,name,id FROM user_info WHERE '.$where_condition." LIMIT $s_point,$list";
                  ...
                  ?><tr>
                      ...
                      <td>
                        <form class="" action="add_friend.php" method="post">
                          <input type="hidden" name="fallow_id" value="<?=$row['id'] //친구삭제할 id?>">
                          <input type="hidden" name="page" value="<?=$page //현제 페이지?>">
          ...
          <table>
            <tr>
              <td width="50" align="center">
                <?php //현제 블록에 표시된 페이지보다 앞페이지가 없을때
                  if ($s_page <=1) { //s_page는 현제블록의 시작페이지를 의미하고
                    // 시자페이지가 1보다 같거나 작을땐
                    $pre = '';// 링크를 표시하지 않고
                  }else { //시작페이지가 1보다 클땐
                    $pre = '이전'; // 이전을 표시한다.
                 ...
                 ?>
                <a href="<?=$addr?>?page=<?=$s_page-1?>"><?=$pre?></a>
              </td>
              <td>
                <?php
              ...
              <td width="50" align="center">
                ...
                <?php
                  if ($e_page >= $pageNum-1) {//블록의 마지막페이지의 값이 총 페이지 개수값과 같거나 크면 '다음' 대신 ''이라고 표시하여준다.
                    $next = '';
                    $last_page = $pageNum;
                  }else {
                    $next = "다음";
